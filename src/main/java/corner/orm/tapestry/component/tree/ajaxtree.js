...
Ajax.Tree.Base.prototype = {
	initialize: function(parent,id,type,data){
		...
		if(parent == null){
			...
			this.root = true;
			if(data && data.nodes){ this.createNodes(data.nodes); }
			return;
		}		
		this.parent = $(parent);
		this.id = id;
	...
	hide: function(el){
		var node = el || this;
		Element.hide(node.element || node);
	},
	...
	toggle: function(){
		if( this.mark.hasClassName('expanded') ){ this.hide(); }
		else if( this.mark.hasClassName('collapsed') ){ this.show(); }
	},
	toggleChildren: function(){
		if( this.mark.hasClassName('expanded') ){ this.hideChildren(); }
		else if( this.mark.hasClassName('collapsed') ){ this.showChildren(); }
	}
};