...
/**
 * 数字转换为英文书写，中文书写格式的类 英文书写格式ＡＮＤ的语法规则：在几十几与百位间加上ＡＮＤ
 * 
 * @author lsq
 * @version $Revision$
 * @since 2.5
...
public class MoneyUtil {
	...
	/**
	 * 将数字转换为英文大写格式．
	 * <p>特殊情况处理:0.00为"ZERO ONLY";0.10为"10/100 ONLY";0.132为"13/100 ONLY"
	 * @param value
	 * @return 英文大写格式
	 */
	public static String parseToEnglishCurrency(String value) {
		...
		for (int i = 0; i < lstrrev.length() / 3; i++) {
			...
			if (!a[i].equals("000")) { // 用来避免这种情况：1000000 = one million
				...
				if (i != 0) {
					...
					// 如:1001 应该为:ONE THOUSAND AND ONE ONLY 要加AND 而不是ONE
					// THOUSAND ONE ONLY
		...
		if (z > -1) {
			...
			//只取两位,其他位数舍去
			if(rstr.length()>2){
				rstr = rstr.substring(0,2);
			}
			
			int pointNum = Integer.parseInt(rstr);
			// 小数点后不为0,则进行处理
			if (pointNum != 0) {
				//小数点前为0,则不加AND只返回小数位
				if(lm.trim().length() == 0){
		...
		//特殊情况 零点几 时,ZERO+小数位+ONLY
		if(lm.trim().length() == 0 && xs.length() == 0){
			return "ZERO ONLY";
		}
