package corner.z7.SevenZip.Compression.Branch;
...
public class BCJ2_x86_Decoder implements ICompressCoder2 {
    ...
    corner.z7.SevenZip.Compression.RangeCoder.BitDecoder[] _statusE8Decoder = new corner.z7.SevenZip.Compression.RangeCoder.BitDecoder[256];
    corner.z7.SevenZip.Compression.RangeCoder.BitDecoder _statusE9Decoder = new corner.z7.SevenZip.Compression.RangeCoder.BitDecoder(kNumMoveBits);
    corner.z7.SevenZip.Compression.RangeCoder.BitDecoder _statusJccDecoder = new corner.z7.SevenZip.Compression.RangeCoder.BitDecoder(kNumMoveBits);
    OutWindow _outStream = new OutWindow();
    corner.z7.SevenZip.Compression.RangeCoder.Decoder _rangeDecoder = new corner.z7.SevenZip.Compression.RangeCoder.Decoder();

    // static final boolean IsJcc(int b0, int b1) {
    //     return ((b0 == 0x0F) && ((b1 & 0xF0) == 0x80));
    // }
    int CodeReal(RecordVector<java.io.InputStream> inStreams, Object useless1, // const UInt64 ** /* inSizes */,
        ...
        for (int i = 0; i < 256; i++) {
            _statusE8Decoder[i] = new corner.z7.SevenZip.Compression.RangeCoder.BitDecoder(kNumMoveBits);
        ...
        for (;;) {
            if ((processedBytes > (1 << 20)) && (progress != null)) {
                ...
                int res = progress.SetRatioInfo(ICompressProgressInfo.INVALID,
                        nowPos64);
    ...
    public int Code(RecordVector<java.io.InputStream> inStreams, // ISequentialInStream **inStreams,
        Object useless_inSizes, // const UInt64 ** /* inSizes */,
        int numInStreams, RecordVector<java.io.OutputStream> outStreams, // ISequentialOutStream **outStreams
