package corner.z7.SevenZip.Compression.LZMA;

import corner.z7.SevenZip.Compression.LZMA.Base;
import corner.z7.SevenZip.Compression.RangeCoder.BitTreeEncoder;
import corner.z7.SevenZip.ICodeProgress;
...
public class Encoder {
    ...
    corner.z7.SevenZip.Compression.LZ.BinTree _matchFinder = null;
    corner.z7.SevenZip.Compression.RangeCoder.Encoder _rangeEncoder = new corner.z7.SevenZip.Compression.RangeCoder.Encoder();
    ...
    void Create() {
        if (_matchFinder == null) {
            corner.z7.SevenZip.Compression.LZ.BinTree bt = new corner.z7.SevenZip.Compression.LZ.BinTree();
            int numHashBytes = 4;
    ...
    void Init() {
        ...
        corner.z7.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isMatch);
        corner.z7.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isRep0Long);
        corner.z7.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isRep);
        corner.z7.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isRepG0);
        corner.z7.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isRepG1);
        corner.z7.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isRepG2);
        corner.z7.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_posEncoders);
    ...
    int GetRepLen1Price(int state, int posState) {
        return corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRepG0[state]) +
        corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRep0Long[(state << Base.kNumPosStatesBitsMax) +
            posState]);
    }
    ...
    int GetPureRepPrice(int repIndex, int state, int posState) {
        ...
        if (repIndex == 0) {
            price = corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRepG0[state]);
            price += corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRep0Long[(state << Base.kNumPosStatesBitsMax) +
                posState]);
        } else {
            price = corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRepG0[state]);

            if (repIndex == 1) {
                price += corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRepG1[state]);
            } else {
                price += corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRepG1[state]);
                price += corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice(_isRepG2[state],
                    repIndex - 2);
            }
        }
    ...
    int GetPosLenPrice(int pos, int len, int posState) {
        ...
        if (pos < Base.kNumFullDistances) {
            ...
            price = _posSlotPrices[(lenToPosState << Base.kNumPosSlotBits) +
                GetPosSlot2(pos)] + _alignPrices[pos & Base.kAlignMask];
        }
    ...
    int GetOptimum(int position) throws IOException {
        ...
        byte matchByte = _matchFinder.GetIndexByte(0 - _repDistances[0] - 1 -
                1);
        ...
        _optimum[1].Price = corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(_state << Base.kNumPosStatesBitsMax) +
                posState]) +
        ...
        int matchPrice = corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(_state << Base.kNumPosStatesBitsMax) +
                posState]);
        int repMatchPrice = matchPrice +
            corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRep[_state]);
        ...
        int normalMatchPrice = matchPrice +
            corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRep[_state]);
        ...
        while (true) {
            ...
            int curAnd1Price = curPrice +
                corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(state << Base.kNumPosStatesBitsMax) +
                    posState]) +
                _literalEncoder.GetSubCoder(position,
                    _matchFinder.GetIndexByte(0 - 2))
                               .GetPrice(!Base.StateIsCharState(state),
            ...
            matchPrice = curPrice +
                corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(state << Base.kNumPosStatesBitsMax) +
                    posState]);
            repMatchPrice = matchPrice +
                corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRep[state]);
            ...
            int numAvailableBytesFull = _matchFinder.GetNumAvailableBytes() +
                1;
            ...
            if (!nextIsChar && (matchByte != currentByte)) {
                ...
                if (lenTest2 >= 2) {
                    ...
                    int nextRepMatchPrice = curAnd1Price +
                        corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) +
                            posStateNext]) +
                        corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRep[state2]);
            ...
            for (int repIndex = 0; repIndex < Base.kNumRepDistances;
                    repIndex++) {
                ...
                if (lenTest < numAvailableBytesFull) {
                    ...
                    if (lenTest2 >= 2) {
                        ...
                        int curAndLenCharPrice = repMatchPrice +
                            GetRepPrice(repIndex, lenTest, state, posState) +
                            corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(state2 << Base.kNumPosStatesBitsMax) +
                                posStateNext]) +
            ...
            if (newLen >= startLen) {
                normalMatchPrice = matchPrice +
                    corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRep[state]);
                                ...
                                int curAndLenCharPrice = curAndLenPrice +
                                    ...
                                    _literalEncoder.GetSubCoder(position +
                                        ...
                                        _matchFinder.GetIndexByte(lenTest - 1 -
                                            1))
                                                   .GetPrice(true,
                                        _matchFinder.GetIndexByte(lenTest -
                                            (curBack + 1) - 1),
                                        _matchFinder.GetIndexByte(lenTest - 1));
                                state2 = Base.StateUpdateChar(state2);
                                posStateNext = (position + lenTest + 1) &
    ...
    boolean ChangePair(int smallDist, int bigDist) {
        ...
        return ((smallDist < (1 << (32 - kDif))) &&
        (bigDist >= (smallDist << kDif)));
    }
    ...
    void WriteEndMarker(int posState) throws IOException {
        ...
        _rangeEncoder.Encode(_isMatch,
            (_state << Base.kNumPosStatesBitsMax) + posState, 1);
    ...
    public void CodeOneBlock(long[] inSize, long[] outSize, boolean[] finished)
        ...
        while (true) {
            ...
            int pos = backRes;
            int posState = ((int) nowPos64) & _posStateMask;
            int complexState = (_state << Base.kNumPosStatesBitsMax) +
                posState;

            if ((len == 1) && (pos == -1)) {
                _rangeEncoder.Encode(_isMatch, complexState, 0);

                byte curByte = _matchFinder.GetIndexByte((int) (0 -
                        _additionalOffset));
                LiteralEncoder.Encoder2 subCoder = _literalEncoder.GetSubCoder((int) nowPos64,
                        _previousByte);

                if (!Base.StateIsCharState(_state)) {
                    byte matchByte = _matchFinder.GetIndexByte((int) (0 -
                    ...
                    if (posSlot >= Base.kStartPosModelIndex) {
                        ...
                        if (posSlot < Base.kEndPosModelIndex) {
                            BitTreeEncoder.ReverseEncode(_posEncoders,
                                baseVal - posSlot - 1, _rangeEncoder,
    ...
    void SetStreams(java.io.InputStream inStream,
        ...
        _repMatchLenEncoder.SetTableSize((_numFastBytes + 1) -
            Base.kMatchMinLen);
    ...
    void FillDistancesPrices() {
        for (int i = Base.kStartPosModelIndex; i < Base.kNumFullDistances;
                i++) {
        ...
        for (int lenToPosState = 0; lenToPosState < Base.kNumLenToPosStates;
                lenToPosState++) {
            int posSlot;
            ...
            for (posSlot = Base.kEndPosModelIndex; posSlot < _distTableSize;
                    posSlot++)
                _posSlotPrices[st + posSlot] += ((((posSlot >> 1) - 1) -
                Base.kNumAlignBits) << corner.z7.SevenZip.Compression.RangeCoder.Encoder.kNumBitPriceShiftBits);
    ...
    public boolean SetLcLpPb(int lc, int lp, int pb) {
        if ((lp < 0) || (lp > Base.kNumLitPosStatesBitsEncodingMax) ||
                (lc < 0) || (lc > Base.kNumLitContextBitsMax) || (pb < 0) ||
    ...
    class LiteralEncoder {
        ...
        class Encoder2 {
            ...
            public void Init() {
                corner.z7.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(m_Encoders);
            }

            public void Encode(
                corner.z7.SevenZip.Compression.RangeCoder.Encoder rangeEncoder,
            ...
            public int GetPrice(boolean matchMode, byte matchByte, byte symbol) {
                ...
                if (matchMode) {
                    for (; i >= 0; i--) {
                        int matchBit = (matchByte >> i) & 1;
                        int bit = (symbol >> i) & 1;
                        price += corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice(m_Encoders[((1 +
                            matchBit) << 8) + context], bit);
                        context = (context << 1) | bit;
                ...
                for (; i >= 0; i--) {
                    int bit = (symbol >> i) & 1;
                    price += corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice(m_Encoders[context],
                        bit);
                    context = (context << 1) | bit;
                }
    ...
    class LenEncoder {
        ...
        public LenEncoder() {
            for (int posState = 0; posState < Base.kNumPosStatesEncodingMax;
                    posState++) {
        ...
        public void Init(int numPosStates) {
            corner.z7.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_choice);
        ...
        public void SetPrices(int posState, int numSymbols, int[] prices, int st) {
            int a0 = corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_choice[0]);
            int a1 = corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_choice[0]);
            int b0 = a1 +
                corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_choice[1]);
            int b1 = a1 +
                corner.z7.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_choice[1]);
            int i = 0;
            ...
            for (; i < (Base.kNumLowLenSymbols + Base.kNumMidLenSymbols);
                    i++) {
                if (i >= numSymbols) {
                    return;
                }
                ...
                prices[st + i] = b0 +
                    _midCoder[posState].GetPrice(i - Base.kNumLowLenSymbols);
            }
    ...
    class LenPriceTableEncoder extends LenEncoder {
        ...
        void UpdateTable(int posState) {
            SetPrices(posState, _tableSize, _prices,
