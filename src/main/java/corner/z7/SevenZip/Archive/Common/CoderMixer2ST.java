...
import corner.z7.SevenZip.ICompressSetInStream;
import corner.z7.SevenZip.ICompressSetOutStream;
import corner.z7.SevenZip.ICompressSetOutStreamSize;
...
public class CoderMixer2ST implements ICompressCoder2, CoderMixer2 {
    ...
    public void SetCoderInfo(int coderIndex, LongVector inSizes,
        LongVector outSizes) {
        // _coders[coderIndex].SetCoderInfo(inSizes, outSizes);
        _coders.get(coderIndex).SetCoderInfo(inSizes, outSizes);
    }

    public int GetInStream(RecordVector<java.io.InputStream> inStreams,
        Object useless_inSizes, // const UInt64 **inSizes,
        int streamIndex, java.io.InputStream[] inStreamRes) {
        ...
        for (i = 0; i < (int) coder.NumInStreams; i++) {
            ...
            int res = GetInStream(inStreams, useless_inSizes, startIndex + i,
                    tmp /* &seqInStream2 */);
    ...
    public int GetOutStream(RecordVector<java.io.OutputStream> outStreams,
        Object useless_outSizes, //  const UInt64 **outSizes,
        int streamIndex, java.io.OutputStream[] outStreamRes) {
        ...
        for (i = 0; i < (int) coder.NumOutStreams; i++) {
            ...
            int res = GetOutStream(outStreams, useless_outSizes,
                    startIndex + i, tmp /* &seqOutStream2 */);
    ...
    public int Code(RecordVector<java.io.InputStream> inStreams,
        ...
        for (i = 0; i < (int) mainCoder.NumInStreams; i++) {
            java.io.InputStream[] tmp = new java.io.InputStream[1];
            int res = GetInStream(inStreams, useless_inSizes, startInIndex + i,
                    tmp /* &seqInStream */);
        ...
        for (i = 0; i < _coders.size(); i++) {
            ...
            try {
                ...
                int res = setOutStreamSize.SetOutStreamSize(coder.OutSizePointers.get(
                            0));
        ...
        if (mainCoder.Coder != null) {
            int res = mainCoder.Coder.Code(seqInStreamsSpec.get(0),
                    seqOutStreamsSpec.get(0), // TBD mainCoder.InSizePointers.get(0),
            ...
            int res = mainCoder.Coder2.Code(seqInStreamsSpec, // &seqInStreamsSpec.Front(
