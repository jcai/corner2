...
import corner.z7.Common.LimitedSequentialInStream;
import corner.z7.Common.LockedInStream;
import corner.z7.Common.LockedSequentialInStreamImp;
...
import corner.z7.SevenZip.Archive.Common.CoderMixer2;
import corner.z7.SevenZip.Archive.Common.CoderMixer2ST;
import corner.z7.SevenZip.Archive.Common.CoderStreamsInfo;
import corner.z7.SevenZip.Archive.Common.FilterCoder;
import corner.z7.SevenZip.HRESULT;
import corner.z7.SevenZip.ICompressCoder;
import corner.z7.SevenZip.ICompressCoder2;
import corner.z7.SevenZip.ICompressFilter;
import corner.z7.SevenZip.ICompressProgressInfo;
import corner.z7.SevenZip.ICompressSetDecoderProperties2;
...
class Decoder {
    ...
    static void ConvertFolderItemInfoToBindInfo(Folder folder,
        BindInfoEx bindInfo) {
        bindInfo.Clear();
        ...
        for (int i = 0; i < folder.Coders.size(); i++) {
            ...
            for (int j = 0; j < coderStreamsInfo.NumOutStreams;
                    j++, outStreamIndex++)
    ...
    int Decode(IInStream inStream, long startPos, LongVector packSizes,
        int packSizesOffset, // const UInt64 *packSizes,
        Folder folderInfo, java.io.OutputStream outStream,
            ...
            for (i = 0; i < numCoders; i++) {
                ...
                if (coderInfo.IsSimpleCoder()) {
                    ...
                    if (altCoderInfo.MethodID.equals(MethodID.k_LZMA)) {
                        decoder = new corner.z7.SevenZip.Compression.LZMA.Decoder(); // NCompress::NLZMA::CDecoder;
                    }
                    ...
                    if (altCoderInfo.MethodID.equals(MethodID.k_BCJ_X86)) {
                        filter = new corner.z7.SevenZip.Compression.Branch.BCJ_x86_Decoder();
                    }
                    ...
                    if (altCoderInfo.MethodID.equals(MethodID.k_Copy)) {
                        decoder = new corner.z7.SevenZip.Compression.Copy.Decoder(); // decoder = new NCompress::CCopyCoder;
                    }
                    ...
                    if (_multiThread) {
                        ...
                        throw new IOException(
                            "Multithreaded decoder is not implemented");
                    } else {
                    ...
                    if (altCoderInfo.MethodID.equals(MethodID.k_BCJ2)) {
                        decoder = new corner.z7.SevenZip.Compression.Branch.BCJ2_x86_Decoder();
                    }
        ...
        for (i = 0; i < numCoders; i++) {
            ...
            for (j = 0; j < numOutStreams; j++, unPackStreamIndex++)
                unPackSizesPointers.add(folderInfo.UnPackSizes.get(
                        unPackStreamIndex));
            ...
            for (j = 0; j < numInStreams; j++, packStreamIndex++) {
                ...
                if (bindPairIndex >= 0) {
                    packSizesPointers.add(folderInfo.UnPackSizes.get(
        ...
        int[] tmp1 = new int[1];
        bindInfo.FindOutStream(bindInfo.OutStreams.get(0),
            tmp1 /* mainCoder */, temp_useless /* temp */);
