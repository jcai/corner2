...
import corner.z7.SevenZip.Archive.Common.OutStreamWithCRC;
...
class FolderOutStream extends java.io.OutputStream {
    ...
    public int Init(ArchiveDatabaseEx archiveDatabase, int ref2Offset,
        int startIndex, BoolVector extractStatuses,
        IArchiveExtractCallback extractCallback, boolean testMode)
    ...
    int OpenFile() throws java.io.IOException {
        ...
        int ret = _extractCallback.GetStream(_ref2Offset + index,
                realOutStream2, askMode);
    ...
    public void write(int b) throws java.io.IOException {
        throw new java.io.IOException(
            "FolderOutStream - write() not implemented");
    }

    public void write(byte[] data, int off, int size)
        throws java.io.IOException //  UInt32 *processedSize
     {
        ...
        while (_currentIndex < _extractStatuses.size()) {
            if (_fileIsOpen) {
                ...
                _outStreamWithHash.write(data, realProcessedSize + off,
                    numBytesToWrite);
                ...
                if (_filePos == fileSize) {
                    ...
                    int res = _extractCallback.SetOperationResult(digestsAreEqual
                            ? IInArchive.NExtract_NOperationResult_kOK
                            : IInArchive.NExtract_NOperationResult_kCRCError);

                    if (res != HRESULT.S_OK) {
                        throw new java.io.IOException(
                            "_extractCallback.SetOperationResult : " + res); // return res;
                    }
                ...
                if (realProcessedSize == size) {
                    ...
                    if (res != HRESULT.S_OK) {
                        throw new java.io.IOException("WriteEmptyFiles : " +
                            res); // return res;
                    }
    ...
    public int FlushCorrupted(int resultEOperationResult)
        throws java.io.IOException {
        while (_currentIndex < _extractStatuses.size()) {
            if (_fileIsOpen) {
