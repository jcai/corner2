...
import corner.z7.SevenZip.Archive.IArchiveExtractCallback;
import corner.z7.SevenZip.Archive.IInArchive;
import corner.z7.SevenZip.Archive.SevenZipEntry;
import corner.z7.SevenZip.Common.LocalCompressProgressInfo;
import corner.z7.SevenZip.Common.LocalProgress;
...
public class Handler implements corner.z7.SevenZip.Archive.IInArchive {
    ...
    public int Extract(int[] indices, int numItems, int testModeSpec,
        IArchiveExtractCallback extractCallbackSpec) throws IOException {
        ...
        for (int ii = 0; ii < numItems; ii++) {
...

            {
                ...
                if (folderIndex == InArchive.kNumNoIndex) {
                    ...
                    // volumeIndex,
                    // #endif
                    fileIndex, InArchive.kNumNoIndex));
                 ...
                 */
                    ...
                    #endif
                     */
                    InArchive.kNumNoIndex, folderIndex));
        ...
        Decoder decoder = new Decoder(// #ifdef _ST_MODE
            false// #else
            // true
            // #endif
            );
        ...
        for (int i = 0; i < extractFolderInfoVector.size();
                i++, currentImportantTotalUnPacked += totalFolderUnPacked) {
            ...
            int result = folderOutStream.Init(database, // #ifdef _7Z_VOL
            ...
            localCompressProgressSpec.Init(progress,
                ICompressProgressInfo.INVALID, currentImportantTotalUnPacked);
             ...
             */
            try {
                result = decoder.Decode(// #ifdef _7Z_VOL
                    // volume.Stream,
                    // #else
                    _inStream, // #endif
                    folderStartPackPos, database.PackSizes, // database.PackSizes.get(packStreamIndex),
                        packStreamIndex, folderInfo, outStream, compressProgress// #ifndef _NO_CRYPTO
                    // , getTextPassword
                    // #endif
                    // #ifdef COMPRESS_MT
                    // , true, _numThreads
            ...
            for (int i = folderInfo.Coders.size() - 1; i >= 0; i--) {
                ...
                for (int j = 0; j < coderInfo.AltCoders.size(); j++) {
                    ...
                    if (altCoderInfo.MethodID.equals(MethodID.k_Copy)) {
                        ...
                        methodName = "Deflate";
                    } else if (altCoderInfo.MethodID.equals(
                                MethodID.k_Deflate64)) {
                    ...
                    if (methodIsKnown) {
                        ...
                        if (altCoderInfo.MethodID.equals(MethodID.k_LZMA)) {
                            if (altCoderInfo.Properties.GetCapacity() >= 5) {
                                ...
                                int dicSize = GetUInt32FromMemLE(altCoderInfo.Properties.data(),
                                        1);
    ...
    public SevenZipEntry getEntry(int index) {
        FileItem item = _database.Files.get(index);
        int index2 = index;
