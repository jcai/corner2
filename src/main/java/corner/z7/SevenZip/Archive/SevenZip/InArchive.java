...
import corner.z7.SevenZip.Common.SequentialOutStreamImp2;
import corner.z7.SevenZip.Common.StreamUtils;
import corner.z7.SevenZip.HRESULT;
import corner.z7.SevenZip.IInStream;
...
class InArchive extends Header {
    ...
    int FindAndReadSignature(IInStream stream, // IInStream *stream,
        ...
        for (;;) {
            ...
            processedSize = ReadDirect(stream, buffer, numPrevBytes,
                    numReadBytes);
    ...
    int ReadArchiveProperties(InArchiveInfo archiveInfo)
        throws IOException {
        for (;;) {
            long type = ReadID();
    ...
    int GetNextFolderItem(Folder folder) throws IOException {
        ...
        for (i = 0; i < numCoders; i++) {
            ...
            for (;;) {
                ...
                int ret = ReadBytes(altCoder.MethodID.ID,
                        altCoder.MethodID.IDSize);
    ...
    int Open(IInStream stream, // IInStream *stream
        long searchHeaderSizeLimit // const UInt64 *searchHeaderSizeLimit
    ) throws IOException {
        Close();
    ...
    int ReadStreamsInfo(ObjectVector<ByteBuffer> dataVector, long[] dataOffset,
        LongVector packSizes, BoolVector packCRCsDefined, IntVector packCRCs,
        ObjectVector<Folder> folders, IntVector numUnPackStreamsInFolders,
        LongVector unPackSizes, BoolVector digestsDefined, IntVector digests)
        throws IOException {
        for (;;) {
            long type = ReadID();
            ...
            case NID.kSubStreamsInfo: {
                int result = ReadSubStreamsInfo(folders,
                        numUnPackStreamsInFolders, unPackSizes, digestsDefined,
                        digests);
    ...
    int ReadHashDigests(int numItems, BoolVector digestsDefined,
    ...
    int ReadPackInfo(long[] dataOffset, // UInt64 &dataOffset,
    ...
    int ReadUnPackInfo(ObjectVector<ByteBuffer> dataVector,
    ...
    int ReadSubStreamsInfo(ObjectVector<Folder> folders,
        IntVector numUnPackStreamsInFolders, LongVector unPackSizes,
    ...
    int ReadAndDecodePackedStreams(long baseOffset, long[] dataOffset,
        ...
        int ret = ReadStreamsInfo(null, dataOffset, packSizes, packCRCsDefined,
                packCRCs, folders, numUnPackStreamsInFolders, unPackSizes,
        ...
        for (int i = 0; i < folders.size(); i++) {
            ...
            int result = decoder.Decode(_stream, dataStartPos, packSizes,
                    packIndex, // &packSizes[packIndex]
                    folder, outStream, null// _ST_MODE , false, 1
                );
            ...
            if (folder.UnPackCRCDefined) {
                if (!CRC.VerifyDigest(folder.UnPackCRC, data.data(),
                            (int) unPackSize)) {
    ...
    int ReadDatabase(ArchiveDatabaseEx database) throws IOException {
        ...
        if (!CRC.VerifyDigest(nextHeaderCRC, buffer2.data(),
                    (int) nextHeaderSize)) {
            throw new IOException("Incorrect Header"); // CInArchiveException(CInArchiveException::kIncorrectHeader);
        }
        ...
        for (;;) {
            ...
            int result = ReadAndDecodePackedStreams(database.ArchiveInfo.StartPositionAfterHeader,
    ...
    int ReadHeader(ArchiveDatabaseEx database) throws IOException {
        ...
        if (type == NID.kMainStreamsInfo) {
            ...
            int result = ReadStreamsInfo(dataVector, ltmp, // database.ArchiveInfo.DataStartPosition,
                    database.PackSizes, database.PackCRCsDefined,
                    database.PackCRCs, database.Folders,
                    database.NumUnPackStreamsVector, unPackSizes,
                    digestsDefined, digests);
