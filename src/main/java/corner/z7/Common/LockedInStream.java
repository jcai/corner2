...
public class LockedInStream {
     ...
     */
    public synchronized int read(long startPos, byte[] data, int size)
    ...
    public synchronized int read(long startPos, byte[] data, int off, int size)
        throws java.io.IOException {
        // NWindows::NSynchronization::CCriticalSectionLock lock(_criticalSection);
        _stream.Seek(startPos, IInStream.STREAM_SEEK_SET);
