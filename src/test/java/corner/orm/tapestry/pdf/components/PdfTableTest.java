...
public class PdfTableTest extends BaseComponentTestCase {
	...
	public void test_component() throws Exception {
		...
		for (int i = 0; i < pageNum; i++) {
			AbstractPdfTableDisplay pdfText = (AbstractPdfTableDisplay) newInstance(
					PdfTable.class, "model", testPage.getPdfTableModel(),
					"source", testPage.getTableSource(), "page", page,
					"clientId", "clientId", "id", "name" + i, "fieldCreator",
					creator);
	...
	private byte[] createPdfTemplateData(int pageNum) throws Exception {
		...
		for (int i = 0; i < pageNum; i++) {
			...
			TextField tf = new TextField(writer, new Rectangle(100, 400, 200,
					500), "name" + i);
