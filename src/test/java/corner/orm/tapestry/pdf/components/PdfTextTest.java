...
public class PdfTextTest extends BaseComponentTestCase {
	...
	public void test_pdfText() throws Exception {
		...
		for (int i = 0; i < pageNum; i++) {
			PdfText pdfText = (PdfText) newInstance(PdfText.class, "value",
					"test pdf text" + i, "page", page, "clientId", "clientId",
					"id", "name" + i,"fieldCreator",creator);
		...
		for (int i = 0; i < pageNum; i++) {
			assertEquals(reader2.getAcroFields().getField("name" + i),
					"test pdf text" + i);
		}
	}
