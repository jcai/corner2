 ...
 *  VERSION: 2.859
 *
 *  Copyright 2008 Andrew M. Andrews III (www.AMA3.com). Some Rights Reserved.
 *  This work licensed under the Creative Commons Attribution-Noncommercial-
 *  Share Alike 3.0 Unported License except in jurisdicitons for which the
 *  license has been ported by Creative Commons International, where the work
 *  is licensed under the applicable ported license instead. For a copy of the
 *  unported license, visit http://creativecommons.org/licenses/by-nc-sa/3.0/
 ...
 *  (www.prototypejs.org), currently version 1.6.0.1.
 *
 *  The ATWidget code and styles in anytime.css have been tested (but not
 *  extensively) on Windows XP in Internet Explorer 7.0, Firefox 2.0, Opera
 *  9.22 and Safari 3.1.  Minor variations in IE are to be expected, due to
 *  its broken box model. Please report any other problems to the author
 *  (URL above).
 *
...
var ATWidget = Class.create( ATConverter,
{
  ...
  initialize: function( $super, id, options )
  {
    ...
    this.labelYear = options.labelDate || 'Year';
    ...
    var title = new Element('h5',{'class':'AtwTitle'});
    ...
    if ( askDate )
    {
      ...
      if ( askDoM )
      {
        ...
        for ( i = 0 ; i < 7 ; i++ )
          tr.appendChild( new Element( 'th', {'class':'AtwDoW AtwDoW'+String(i+1)}).update(this.dAbbr[i]));
        ...
        totH += domTable.getHeight();
        shownFields++;
        divsHigh++;

      } // if ( askDoM )

      totW += dateDiv.getWidth();
      divsWide++;
    ...
    if ( askTime )
    {
      ...
      if ( askHour )
      {
        ...
        var hourDiv = new Element('div',{'class':'AtwHrs'});
        timeDiv.appendChild(hourDiv);

        hourDiv.appendChild((new Element('h6',{'class':'AtwLbl AtwLblHr'})).update(lab) );

        var amDiv = new Element('div',{'class':'AtwHrsAm'});
        hourDiv.appendChild( amDiv );
        var pmDiv = new Element('div',{'class':'AtwHrsPm'});
        hourDiv.appendChild( pmDiv );
      ...
      if ( askMinute )
      {
        ...
        var minutesDiv = new Element('div',{'class':'AtwMins'});
        timeDiv.appendChild(minutesDiv);
        minutesDiv.appendChild((new Element('h6',{'class':'AtwLbl AtwLblMin'}) ).update(lab) );
        var tensDiv = new Element('div',{'class':'AtwMinsTens'});
        minutesDiv.appendChild(tensDiv);
      ...
      if ( askSec )
      {
        ...
        var secondsDiv = new Element('div',{'class':'AtwSecs'});
        timeDiv.appendChild(secondsDiv);
        secondsDiv.appendChild((new Element('h6',{'class':'AtwLbl AtwLblSec'}) ).update(lab) );
        var tensDiv = new Element('div',{'class':'AtwSecsTens'});
        secondsDiv.appendChild(tensDiv);
    ...
    if ( options.labelTitle )
      title.insert( options.labelTitle );
    else if ( shownFields > 1 )
      title.insert( 'Select a '+(askDate?(askTime?'Date and Time':'Date'):'Time') );
    else
      title.insert( '&nbsp;' );
    ...
    {
      totH += title.getHeight() + (divsHigh*3);
      totW += (divsWide*3);
    }
    else
    {
      totH += title.getHeight() + divsHigh + 1;
      totW += (divsWide*2);
    }
  ...
  upd: function(cancelAjax)
  {
    ...
    $$('#'+this.id+' .AtwWk').each(
      ...
      {
        dow = 0;
        for ( td = row.down(); dow < 7 ; td = td.next() )
        {
          var a = td.down('a');
  ...
  function(loadEvent)
  {
    ...
    $$('div.AtwWidget').each(
      function(elem)
      {
        ...
        if ( inst.pop && ( inst.parentNode != document.body ) )
        {
